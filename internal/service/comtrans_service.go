...
func (s *ComtransService) GetComtransEngines(ctx *fasthttp.RequestCtx, lang string) model.Response {
	data, err := s.repository.GetComtransEngines(ctx, lang)
	if err != nil {
		return model.Response{
			Status: 500,
			Error:  err,
...
func (s *ComtransService) GetComtrans(ctx *fasthttp.RequestCtx, userID int, targetUserID string, brands, models, regions, cities,
	...
	data, err := s.repository.GetComtrans(ctx, userID, targetUserID, brands, models, regions, cities,
		transmissions, engines, fuel_types,
		colors, dealers, year_from, year_to,
